...
│   ├── 复现/                  # Mathematica 主程序与模块包（内部保持同目录相对引用）
│   │   ├── 主程序.nb / 主程序.m
│   │   ├── 加载模块.m
│   │   ├── 拓扑缺陷模块包.m
│   │   ├── 液晶排列模式模块包.m
...
- **src/复现/**: Mathematica 主程序与模块包，实现液晶斯格明子形态的数值模拟（Néel 斯格明子、反斯格明子 Anti-Skyrmion、双半子 Bimeron 等）。六个 `.m` 模块包位于同一目录，`加载模块.m` 通过 `Get[...]` 相对引用同目录模块。
...
```mathematica
(* 打开 src/复现/主程序.nb，或在工作目录内加载模块 *)
Get["复现/加载模块.m"]   (* 加载模块（需在 src/ 目录下） *)
Get["复现/主程序.m"]      (* 运行主程序 *)
```

> 注意：`.m` 模块包间的 `Get[...]` 引用为同目录相对引用，请在 `src/` 目录下运行，保持 `src/复现/` 内部结构不变。
